...
Weighted_Mean_Var<- function(weighted_sum,weights,sum_of_weights,sum_square,center_bin,width)
{
  ...
  #weighted_var=ceiling((sum(a[lower:upper])/sum(weighted_z[lower_z:upper_z]))-(weighted_mean^2) )#best #########original
  ...
  meanvar_values <- c(weighted_mean,1/sqrt(weighted_var))#############original
  meanvar_values
}
########################
New_Mean_Variance<-function(rep1,rep2,ordered_scores)#binning based on similar numbers of rep1 in each rep(all zeros in one bin all 0.02 in one bin,...) 
{
  aa=unique(ordered_scores[,1])
  l_aa=length(aa)
  bin=l_aa
  epsilon=0.0001
  mean_var=matrix(nrow=bin,ncol=2)
  scores=ordered_scores
  for(i in 1:bin)
  {
    print(i)
    temp=subset(scores, scores[,1]==aa[i])
    mean_var[i,1]=mean(temp[,2])
    temp_2=var(temp[,2])+epsilon
    mean_var[i,2]=1/sqrt(temp_2)
    #scores=scores[-(1:length(temp[,1])),]
  }

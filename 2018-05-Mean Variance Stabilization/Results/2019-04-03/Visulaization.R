...
######################################################parameter setting##################
setwd("/Users/faezehbayat/Documents/FINAL experiments(cedar)/2019-04-03")
library(ggplot2)
library(plyr)
library(dplyr)
library(reshape2)
library(ggplot2)
data <- read.csv("/Users/faezehbayat/Documents/FINAL experiments(cedar)/2019-04-03/Total results.csv")
step.data <- data[data$Method == "SDPM",]
step.data <- step.data[step.data$Evaluation.type == "Gene expression analysis",]
data <- step.data
...
ggsave("gene_expression_Bin_band_pearson.pdf", p, width=20, height=10, units="cm",scale=1)
ggsave("Bin_Band_Pearson.pdf", p, width=20, height=10, units="cm",scale=1)
##############
data <- read.csv("/Users/faezehbayat/Documents/FINAL experiments(cedar)/2019-04-03/Total results.csv")
step.data <- data[data$Method == "SDPM",]
step.data <- step.data[step.data$Evaluation.type == "Differential expression",]
...
step.data <- mean_data
p=ggplot(step.data) + aes(x = as.factor(Beta), shape = as.factor(Bin.size), y = avg_eval) + geom_point(stat="identity",aes( color=as.factor(Bin.size))) + theme_bw()+labs(x = "\nBand width",y="\nLinearity score")
ggsave("differrential_Bin_band_pearson.pdf", p, width=20, height=10, units="cm",scale=1)
##################################
setwd("/Users/faezehbayat/Documents/FINAL experiments(cedar)")
...
p=ggplot(step.data) + aes(x = as.factor(Beta), shape = as.factor(Bin.size), y = avg_eval) + geom_point(stat="identity",aes( color=as.factor(Bin.size))) + theme_bw()+labs(x = "\nBand width",y="\nLog likelihood")
p
ggsave("parameter_setting_likelihood.pdf", p, width=12, height=10, units="cm",scale=1)
...
library(reshape2)
library(ggplot2)
data <- read.csv("/Users/faezehbayat/Documents/FINAL experiments(cedar)/new_assays_gene_expression.csv")
step.data <- data[data$Evalation.type == "Gene expression analysis",]
step.data <- data[step.data$Dataset == "H3K36me3 H1-hESC",]
data <- step.data
mean_data <- summarize(group_by(data, Beta, Width, Bin.size), avg_eval=mean(Evaluation.Value))
step.data <- mean_data
...
p=ggplot(step.data) + aes(x = as.factor(Beta), shape = as.factor(Bin.size), y = avg_eval) + geom_point(stat="identity",aes( color=as.factor(Bin.size))) + theme_bw()+labs(x = "\nBand width",y="\nLinearity score")
p
ggsave("new_assays_gene_expression.pdf", p, width=12, height=10, units="cm",scale=1)
